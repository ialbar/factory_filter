...
static float_t f2er_high_pass_ops_filtered(struct f2er *f2er, float_t input)
{
    uint16_t i=0;
    float_t temp=0;
...
static struct f2er_ops f2er_ops = {
    .filtered = f2er_high_pass_ops_filtered,
};


void f2er_high_pass_init(struct f2er_high_pass *f2er_high_pass)
{
    f2er_init(&f2er_high_pass->f2er);
    f2er_high_pass->f2er.ops=&f2er_ops;
}
